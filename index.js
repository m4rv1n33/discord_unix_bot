...
class WebhookLogger {
  ...
  async sendStatusEmbed(
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Timezones",
          value: `${stats.timezoneCount} configured`,
          inline: true,
        },
  ...
  getRecentActivity() {
    ...
    const recent = this.recentCommands.filter(
      (cmd) => cmd.time > thirtyMinutesAgo
    );
    ...
    try {
      ...
      if (logger)
        logger.info(
          ...
          } timezone(s) from volume storage`
        );
    } catch (error) {
...
client.once("ready", async () => {
  ...
  await logger.sendToWebhook(
    `Bot Started Successfully\n` +
      ...
      `Storage: ${
        usingPersistentStorage ? "Persistent" : "Local (at risk)"
      }\n` +
      `Status reports: ${
        process.env.LOG_WEBHOOK_URL ? "Scheduled every 30 minutes" : "Disabled"
      }\n` +
  ...
  console.log(
    `Status reports: ${
      process.env.LOG_WEBHOOK_URL ? "Enabled (every 30 minutes)" : "Disabled"
    }`
  );
});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isUserContextMenuCommand()) {
    ...
    logger.info(
      `Context menu: ${command} used by ${interaction.user.tag} on ${targetUser.tag}`
    );
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId.startsWith("set_tz_modal_")) {
      ...
      logger.info(
        `Admin ${interaction.user.tag} set timezone for user ${targetUserId} to ${timezone}`
      );
  ...
  logger.debug(
    `Command: /${command} by ${interaction.user.tag} in ${
  ...
  if (
    ...
    command === "backup-timezones" ||
    command === "view-logs" ||
    command === "force-status"
  ) {
    if (!checkAdmin(interaction)) {
      return interaction.reply({
        content: "This command requires administrator permissions.",
  ...
  if (command === "unix-time") {
    ...
    logger.debug(
      `Processing /unix-time: ${timeInput} ${
        dateInput || "(no date)"
      } for ${userTz}`
    );
    ...
    try {
      ...
      const ts = m.unix();
      logger.info(
        `User ${interaction.user.tag} converted ${timeInput} ${
          dateInput || ""
        } to timestamp: ${ts}`
      );
  ...
  if (command === "set-timezone") {
    ...
    logger.info(
      `User ${interaction.user.tag} attempting to set timezone to: ${tz}`
    );
  ...
  if (command === "storage-status") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**Bot Storage Information**\nLoaded from: \`${loadedSource}\``
      )
      .addFields(
        {
          name: "Timezone Count",
          value: `${stats.timezoneCount} users`,
          inline: true,
        },
        {
          name: "Persistent Storage",
          value: stats.usingPersistentStorage ? "Yes" : "No (data at risk)",
          inline: true,
        },
        {
          name: "Volume Writable",
          value: stats.volumeWritable ? "Yes" : "No",
          inline: true,
        },
        {
          name: "Primary File",
          value: stats.fileExists ? `${stats.fileSize} bytes` : "Missing",
          inline: true,
        },
        {
          name: "Backup File",
          value: stats.backupExists ? "Exists" : "Missing",
          inline: true,
        },
        {
          name: "Context",
          value: `${stats.guild}\nUser ID: ${stats.userId}\nAdmin: ${
            stats.isAdmin ? "Yes" : "No"
          }`,
          inline: true,
        }
      )
  ...
  if (command === "backup-timezones") {
    ...
    logger.info(
      `Admin ${interaction.user.tag} downloaded timezone backup (${count} users)`
    );
  ...
  if (command === "view-logs") {
    const logInfo = {
      ...
      botUptime: moment(client.readyAt).fromNow(),
      guildCount: client.guilds.cache.size,
      nextStatus: logger.lastStatusTime
        ? moment(logger.lastStatusTime).add(30, "minutes").format("HH:mm:ss")
        : "Not scheduled",
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "Timezones Loaded",
          value: `${logInfo.timezonesLoaded} users`,
          inline: true,
        },
        { name: "Storage", value: logInfo.storageType, inline: true },
        {
          name: "Guilds",
          value: `${logInfo.guildCount} servers`,
          inline: true,
        },
...
client.on("guildCreate", (guild) => {
  if (logger) {
    logger.success(
      `Joined new guild: ${guild.name} (${guild.id}) - Members: ${guild.memberCount}`
    );
  }
});
